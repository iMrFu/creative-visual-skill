...
2. **风格匹配推荐 (Style Selection)**: 智能自动匹配推荐。支持双轨匹配：在启用大模型（V2）时，调用 LLM 充当“视觉创意总监”分析文章与风格内涵并给出包含“经典契合”或“跨界碰撞”策略的专业创意策划理由；离线或大模型故障时自动降级为 V1 规则打分算法，根据文章关键词与风格 tags 的交集打分匹配。
...
9. **视觉钩子与构图策略 (Visual Hooks - V3)**: 新增。在风格匹配后、生图前，分析文章情绪张力并选择最具点击吸引力的钩子和构图策略，实施“美学融合保护”重新编写视觉概念。
...
* **`config.json`**: 模块核心配置文件。
* **`styles/style_library.md`**: 风格模板库，通过 `## Style` 标题与 ```json 代码块扩展。
* **`styles/hook_library.md`**: [NEW] 视觉钩子与构图策略库。
* **`hook_designer.py`**: [NEW] 视觉钩子设计器模块。
* **`workflows/`**: 预置的 `sdxl_basic.json` 和 `flux_basic.json` 本地生图工作流。
* **`tests/`**: 包含自动化测试套件（新添 `test_hook_designer.py`）。
...
```json
{
  ...
  "visual_concept": "重写后的完整英文视觉概念描述，直接用于生图，包含媒介、构图、主体和画面细节",
  "visual_concept_cn": "中文视觉概念摘要",
  "hook_rationale": "策略选择理由与风格美学融合说明"
}
```
